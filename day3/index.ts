...
const partOne = (lines: Array<string>) => {
  ...
  const epsilonBinary: Array<number> = gammaBinary.map((x) =>
    x === 1 ? 0 : 1
  );
...
const partTwo = (lines: Array<string>): number => {
  ...
  while (oxygenRatingLines.length > 1 && index < countActiveBits.length) {
    ...
    const mostCommonBit =
      countActiveBits[index] >=
      oxygenRatingLines.length - countActiveBits[index]
        ? 1
        : 0;

    oxygenRatingLines = oxygenRatingLines.filter(
      (x) => Number(x[index]) === mostCommonBit
    );
    index++;
  }
  ...
  while (scrubberRatingLines.length > 1 && index < countActiveBits.length) {
    ...
    const leastCommonBit =
      countActiveBits[index] >=
      scrubberRatingLines.length - countActiveBits[index]
        ? 0
        : 1;

    scrubberRatingLines = scrubberRatingLines.filter(
      (x) => Number(x[index]) === leastCommonBit
    );
    index++;
  }
